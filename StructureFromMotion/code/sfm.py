...
def main():
  ...
  # We assume that the correct solution is the one that gives the most points in front of both cameras.
  max_points = 0
  best_pose = -1

  # this pose is assumed to be the transformation from global space to image space
  ...
  for i, (R, t) in enumerate(possible_relative_poses):
    ...
    if num_points > max_points:
      ...
      best_pose = i
  R_best, t_best = possible_relative_poses[best_pose]

  # Set the image poses in the images (image.SetPose(...))
  e_im1.SetPose(np.eye(3), np.zeros(3))
  e_im2.SetPose(R_best.T, -1 * R.T @ t)
